<template>
  ...
  <div class="barber-form-container">
    <h2 class="barber-title">
      {{ isEditMode ? 'Edit Haircut Crew' : 'Add New Haircut Crew' }}
    </h2>
    ...
    <form @submit.prevent="submitForm" class="barber-form" enctype="multipart/form-data">
      <div class="barber-row">
        ...
        <div class="barber-form-group">
          <label>Photo (upload file) <span class="barber-required" v-if="!isEditMode">*</span></label>
          ...
          <input
            ...
            :required="!isEditMode"
          />
          <div v-if="isEditMode && existingPhotoUrl" style="margin-bottom: 10px;">
            <img
      ...
      <button type="submit" class="barber-submit-btn">
        {{ isEditMode ? 'Update Barber' : 'Save Barber' }}
      </button>
    </form>
...
export default {
  ...
  mounted() {
    ...
    if (this.$route.params.id) {
      ...
      this.barberId = this.$route.params.id;
  ...
  methods: {
    ...
    async loadBarber(id) {
      try {
        const res = await fetch(`http://didar.intelsofts.com/Laravel_Vue/B_Backend/public/api/barbers/${id}`);
        const data = await res.json();
        this.form = {
          ...data,
          photo: null, // prevent setting raw photo path
        };
        this.existingPhotoUrl = data.photo ? `http://didar.intelsofts.com/Laravel_Vue/B_Backend/public/${data.photo}` : '';
      } catch (error) {
    ...
    async submitForm() {
      try {
        ...
        const url = this.isEditMode
          ? `http://didar.intelsofts.com/Laravel_Vue/B_Backend/public/api/barbers/${this.barberId}`
          : `http://didar.intelsofts.com/Laravel_Vue/B_Backend/public/api/barbers`;
